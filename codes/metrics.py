...
def sample_l2_paper(y, mechanism, sample_num=2 ** 22):
    player_num, value_range, _ = y.shape
    max_value = value_range - 1

    # analytic solution: bid = (N-1)/N * v or bid = v
    analytic_rate = (player_num - 1) / player_num if mechanism == 'first' else 1.0

    L2_per_bidder = torch.zeros(player_num).to(y.device)

    batch = 2 ** 10
    assert sample_num % batch == 0
    iter_num = int(sample_num / batch)
    for _ in range(iter_num):
        values = torch.rand(size=[batch]).to(y.device)
        analytic_solution = values * analytic_rate

        discrete_values = torch.round(max_value * values).int()
        predicted_strategy = y[:, discrete_values, :]  # n*batch*V_bid
        # sample a bid
        predicted_bid = torch.distributions.Categorical(probs=predicted_strategy).sample()  # n*batch
        normalized_bid = predicted_bid / max_value

        # l2
        L2 = (normalized_bid - analytic_solution) ** 2  # n*batch
        L2_per_bidder += L2.sum(dim=-1)  # n
    # mean
    L2_per_bidder = torch.sqrt(L2_per_bidder / sample_num)  # n

    return L2_per_bidder.mean()


def sample_l2_distance(y: torch.Tensor,
                       mechanism: str,
                       k=100):
    r"""
        ...
        Args:
           ...
           mechanism: 'first' or 'second'
           k: sample num, default 10

        Returns:
            :math:`L_2`
    """
    player_num, value_range = y.shape[:2]
    max_value = value_range - 1
    ...
    rate = (player_num - 1) / player_num if mechanism == 'first' else 1
    analytic = torch.arange(value_range).to(predict.device) * rate  # V
    analytic = analytic.view(1, value_range, 1).repeat(player_num, 1, k)  # n*V*k

    norm_gap = (predict - analytic) / max_value  # n*V*k
    square_dist = (norm_gap ** 2).mean(dim=-1)  # n*V
    l2_dist = torch.sqrt(square_dist.sum(dim=-1) / value_range)   # n

    return torch.mean(l2_dist), torch.std(l2_dist)


def sample_l2_distance_old(value_hist: torch.Tensor,
                       y: torch.Tensor,
