...
def get_model_name(args):
    ...
    model_name += '' if args.from_pretrain else '_r'
    if args.requires_value:
        model = f'{model_name}+NV.' + args.query_style.split('_')[-1]
    else:
        model = model_name if args.use_bert else 'MLP'
    ...
    return f'{mechanism_name}_{distribution_name}_{dataset_aug}_n={args.max_player}_v={args.valuation_range - 1}_{model}_{bert_onoff}_{loss_name}_batch={args.batch_size}_lr={args.lr}_{update_mode}_epoch={args.max_epoch}'
